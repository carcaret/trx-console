  ...
  public Service getService(ServiceType type) {
    for (TxStatistics.Service service : statistics.getService()) {
      if (service.getName().equals(type.getUri())) {
        AggregateValues client = new AggregateValuesImpl(service.getSuccessClientAvg(),
            service.getSuccessClientMin(), service.getSuccessClientMax());
        AggregateValues server = new AggregateValuesImpl(service.getSuccessServerAvg(),
            service.getSuccessServerMin(), service.getSuccessServerMax());
        ErrorCount errors = new ErrorCountImpl(service.getRequesterErrorCounter(),
            service.getResponderErrorCounter());
        LastRequest last = new LastRequestImpl(service.getLastRequest().isSuccess(),
            service.getLastRequest().getFinishTime(), service.getLastRequest().getResult(),
    ...
    throw new IllegalArgumentException(String.format("Service type '%s' not found", type.getUri()));
  }
